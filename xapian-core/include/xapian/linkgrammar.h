...
class XAPIAN_VISIBILITY_DEFAULT LinkGrammar {
  ...
  public:
     ...
     *  @param sentence     The sentence for which the linkage diagram is
                required.
     */
    std::string get_linkage_diagram_string(const std:: string & sentence);
    ...
    std::string pos_to_string(const unsigned pos_type_value,
                    bool null_string_for_none_pos = true);
     ...
     */
    std::string get_constituent_tree_string(const std::string & sentence);

  private:
    // The language of the dictionary.
    std::string Language;

    // The maximum time allowed to take by linkgrammar parser.
    int Time;
};
