                    ...
                    const string & prefix, bool with_positions)
{
    ...
    // get_pos_sentence() shall be empty.
    ...
    for (list<LinkGrammar::pos_info_s>::iterator it = pos_info.begin();
        ...
        // It doesn't makes sense to stem Noun Phrases, since they are usually
        // combination of more than one word with '#' in-between.
        // However we don't want that they are not stored at all, if the
        // stemming strategy is set to STEM_ALL or STEM_ALL_Z.
        if (pos.compare("NOUNPHRASE") == 0 &&
                strategy != TermGenerator::STEM_NONE) {
            if (strategy != TermGenerator::STEM_SOME) {
                if (with_positions) {
