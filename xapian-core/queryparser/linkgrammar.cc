...
LinkGrammar::LinkGrammar(const string & language, const int seconds)
{
     ...
     ********************************************************************/
    Adjective.insert("a");
    Adjective.insert("a-c");
    Adjective.insert("a-s");
...
LinkGrammar::get_pos_description_string(const string & sentence)
{
    ...
    for (it = pos_info.begin(); it != pos_info.end(); it++) {
        ...
        pos_description << pos_to_string(it->pos, false) << endl;
    }
    return pos_description.str();
}
...
{
    if (Noun.find(subscript) != Noun.end())
        return NOUN;

    if (Verb.find(subscript) != Verb.end())
        return VERB;

    if (Adjective.find(subscript) != Adjective.end())
        return ADJECTIVE;

    if (Adverb.find(subscript) != Adverb.end())
        return ADVERB;
